...
describe('Create mask from a GREY image', function () {
  it('From GREY image give a mask image', function () {
    let image = new _common.Image(5, 1, [0, 63, 127, 191, 255], {
      kind: 'GREY'
    });
    expect(image.mask().data[0]).toBe(0b00011000);
    expect(image.mask({
      threshold: 0.9
    }).data[0]).toBe(0b00001000);
    expect(image.mask({
      threshold: 128
    }).data[0]).toBe(0b00011000);
    expect(image.mask({
      threshold: '50%'
    }).data[0]).toBe(0b00011000);
    expect(image.mask({
...
describe('Create a mask from a greyA image', function () {
  let image = new _common.Image(4, 1, [255, 255, 0, 255, 255, 0, 0, 0], {
    kind: 'GREYA'
  });
...
describe('Create a mask from a greyA image using percentile algorithm', function () {
  it('should give the right result', function () {
    let image = new _common.Image(4, 1, [0, 255, 63, 255, 127, 255, 255, 255], {
      kind: 'GREYA'
    });
    let mask = image.mask({
      algorithm: 'percentile',
