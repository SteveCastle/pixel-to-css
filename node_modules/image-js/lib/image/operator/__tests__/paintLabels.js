...
describe.skip('paintLabels', function () {
  it('should yield the painted image', function () {
    let image = new _common.Image(150, 150, {
      kind: 'RGBA'
    });
    let labels = ['1', '2'];
    ...
    image.paintLabels(labels, positions, options); // how to have any test that can work on travis ?

    let means = image.mean;
    expect(means[0]).toBeGreaterThan(0);
    expect(means[1]).toBeGreaterThan(0);
    expect(means[2]).toBeGreaterThan(0); // image.save('test.png');
  });
});