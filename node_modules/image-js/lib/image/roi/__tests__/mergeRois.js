...
test('merge two rois', async () => {
  const img = await (0, _common.load)('BW7x7.png');
  ...
  expect(roi.surface).toBe(17); // Second roi shouldn't exist anymore
...
test('errors', async () => {
  ...
  const mask = img.mask(0.5);
  roiManager.fromMask(mask);
  expect(() => roiManager.mergeRois(1)).toThrow(/Roi ids must be an array of integers/);
  expect(() => roiManager.mergeRois([1.4])).toThrow(/Roi ids must be an array of integers/);
  expect(() => roiManager.mergeRois([1])).toThrow(/Roi ids must have at least two elements/);
  expect(() => roiManager.mergeRois([1, 1])).toThrow(/Roi ids must be all different/);
